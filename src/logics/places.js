export const places = [
  {
    id: 1,
    name: "Little Saint James Island",
    image: "/images/places/stjames.jpg",
    descriptions: [
      "A pristine private Caribbean island paradise.",
      "An exclusive retreat for the world's elite.",
      "A controversial property with a dark past.",
      "The site of a sophisticated blackmail operation.",
      "A front for an intelligence agency honey trap.",
      "The central hub of a global pedophile ring."
    ],
    npcs: [
      { name: "Ghislaine", description: "The socialite who managed recruitment.", special: "", favProduct: "Pizza" },
      { name: "Jeffrey", description: "The island's owner and blackmail collector.", special: "", favProduct: "Hotdog" },
      { name: "Virginia", description: "The victim who exposed the trafficking.", special: "", favProduct: "Cheese" },
      { name: "Les", description: "The pilot for secret client meetings.", special: "", favProduct: "Pasta" },
      { name: "Alexander", description: "The intelligence operative running blackmail.", special: "", favProduct: "Ice cream" },
      { name: "Andrew", description: "The royal with dark island secrets.", special: "", favProduct: "Walnut" }
    ],
    secretLocation: {
      name: "Underwater Temple",
      description: "A submerged temple with hidden chambers accessible only during low tide, perfect for making people disappear."
    },
    partyLocation: {
      name: "The Blue & White Temple",
      description: "The island's main temple, adorned with pagan symbols, where elites perform rituals and indulge in their darkest desires."
    },
    harvestLocation: {
      name: "Neighboring Islands",
      description: "Impoverished local communities where vulnerable children are sourced and trafficked to the island."
    }
  },
  {
    id: 2,
    name: "Vatican City",
    image: "/images/places/vatican.jpg",
    descriptions: [
      "The spiritual heart of Catholicism.",
      "A sovereign city-state with priceless treasures.",
      "A secretive organization above national laws.",
      "Headquarters of a global intelligence network.",
      "Control center for a one-world religion plot.",
      "Seat of an ancient bloodline's power."
    ],
    npcs: [
      { name: "Jorge", description: "The Pope overseeing global secrets.", special: "", favProduct: "Cheese" },
      { name: "Tarcisio", description: "The Secretary of State managing diplomacy.", special: "", favProduct: "Pasta" },
      { name: "Luca", description: "The Swiss Guard protecting secrets.", special: "", favProduct: "Pizza" },
      { name: "Ratzinger", description: "The former Pope pulling strings.", special: "", favProduct: "Hotdog" },
      { name: "Loredana", description: "The archivist who knows too much.", special: "", favProduct: "Walnut" },
      { name: "Alois", description: "The Jesuit coordinating intelligence.", special: "", favProduct: "Ice cream" }
    ],
    secretLocation: {
      name: "Catacomb of Lost Souls",
      description: "Forgotten catacombs beneath the Vatican where dissenters and inconvenient witnesses have been 'buried' for centuries."
    },
    partyLocation: {
      name: "The Papal Apartments",
      description: "The private residence of the Pope, where select cardinals and global elites gather for blasphemous rituals."
    },
    harvestLocation: {
      name: "Catholic Orphanages",
      description: "A global network of orphanages and schools where vulnerable children are harvested for the Church's purposes."
    }
  },
  {
    id: 3,
    name: "Tel Aviv",
    image: "/images/places/tell.jpg",
    descriptions: [
      "A vibrant Mediterranean city with beaches.",
      "Israel's financial and technological hub.",
      "A major intelligence monitoring center.",
      "Global headquarters for Mossad operations.",
      "Control center for media market manipulation.",
      "Earthly HQ for the Zionist global conspiracy."
    ],
    npcs: [
      { name: "Yossi", description: "A startup founder in surveillance tech.", special: "", favProduct: "Pasta" },
      { name: "Miriam", description: "A Mossad officer recruiting assets.", special: "", favProduct: "Walnut" },
      { name: "David", description: "A cybersecurity expert for digital warfare.", special: "", favProduct: "Ice cream" },
      { name: "Sarah", description: "A banker moving illicit funds.", special: "", favProduct: "Hotdog" },
      { name: "Ehud", description: "A private intelligence agency director.", special: "", favProduct: "Cheese" },
      { name: "Benjamin", description: "The leader coordinating global ops.", special: "", favProduct: "Pizza" }
    ],
    secretLocation: {
      name: "Old Jaffa Tunnels",
      description: "Ancient smuggler's tunnels beneath the old port, now used for 'enhanced interrogation' and making problems vanish."
    },
    partyLocation: {
      name: "Rothschild Boulevard Penthouses",
      description: "Luxurious high-rise apartments where the Israeli elite and international financiers host decadent parties."
    },
    harvestLocation: {
      name: "Gaza",
      description: "The besieged territory where conflict and desperation provide a steady supply of vulnerable victims."
    }
  },
  {
    id: 4,
    name: "London",
    image: "/images/places/london.jpg",
    descriptions: [
      "The historic UK capital with the Royal Family.",
      "A global financial and corporate hub.",
      "Nexus of secret societies controlling finance.",
      "HQ of the global banking cartel.",
      "Seat of the Crown Corporation hidden power.",
      "Capital of the Reptilian hybrid bloodlines."
    ],
    npcs: [
      { name: "Elizabeth", description: "The monarch presiding over rituals.", special: "", favProduct: "Ice cream" },
      { name: "Rothschild", description: "The banking heir controlling markets.", special: "", favProduct: "Pizza" },
      { name: "Winston", description: "The Five Eyes intelligence coordinator.", special: "", favProduct: "Walnut" },
      { name: "Jacob", description: "The Freemason Grand Master.", special: "", favProduct: "Hotdog" },
      { name: "Savile", description: "The entertainer procuring for rings.", special: "", favProduct: "Cheese" },
      { name: "Charles", description: "The royal with the Reptilian bloodline.", special: "", favProduct: "Pasta" }
    ],
    secretLocation: {
      name: "Sewer Network Beneath Parliament",
      description: "Victorian-era tunnels beneath the government buildings where inconvenient politicians and journalists have met their end."
    },
    partyLocation: {
      name: "Buckingham Palace",
      description: "The royal residence where the Crown hosts depraved parties for visiting dignitaries and bloodline members."
    },
    harvestLocation: {
      name: "Northern Care Homes",
      description: "Privatized elderly care facilities across northern England where vulnerable patients are harvested."
    }
  },
  {
    id: 5,
    name: "New York City",
    image: "/images/places/ny.jpg",
    descriptions: [
      "The city that never sleeps, a cultural melting pot.",
      "Home to Wall Street and the United Nations.",
      "HQ of media conglomerates shaping perception.",
      "Control center for deep state operations.",
      "Site of alien technology reverse-engineering.",
      "Location of the interdimensional stargate portal."
    ],
    npcs: [
      { name: "Donald", description: "The real estate mogul and intelligence hub.", special: "", favProduct: "Walnut" },
      { name: "Jeffrey", description: "The media executive controlling narratives.", special: "", favProduct: "Pasta" },
      { name: "Michael", description: "The Wall Street market manipulator.", special: "", favProduct: "Cheese" },
      { name: "Rudy", description: "The former mayor covering up 9/11.", special: "", favProduct: "Pizza" },
      { name: "Epstein", description: "The financier running blackmail ops.", special: "", favProduct: "Hotdog" },
      { name: "Rockefeller", description: "The New World Order patriarch.", special: "", favProduct: "Ice cream" }
    ],
    secretLocation: {
      name: "Abandoned Synagogue Tunnels",
      description: "A network of forgotten mikvah tunnels beneath the Lower East Side, perfect for making whistleblowers disappear without a trace."
    },
    partyLocation: {
      name: "Trump Tower Penthouse",
      description: "The gilded penthouse where real estate moguls and financiers host parties with underage guests."
    },
    harvestLocation: {
      name: "Homeless Shelters",
      description: "The city's vast network of shelters where undocumented and vulnerable people disappear without questions."
    }
  },
  {
    id: 6,
    name: "Hollywood",
    image: "/images/places/hollywood.jpg",
    descriptions: [
      "The entertainment capital of the world.",
      "A global cultural influence powerhouse.",
      "A propaganda machine shaping public opinion.",
      "Recruitment center for MK Ultra programs.",
      "HQ of the celebrity-sacrificing satanic cult.",
      "The studio programming the simulation matrix."
    ],
    npcs: [
      { name: "Harvey", description: "The producer trading roles for favors.", special: "", favProduct: "Hotdog" },
      { name: "Steven", description: "The director with deep state ties.", special: "", favProduct: "Pizza" },
      { name: "Oprah", description: "The media mogul controlling narratives.", special: "", favProduct: "Cheese" },
      { name: "Madonna", description: "The pop icon involved in occult rituals.", special: "", favProduct: "Pasta" },
      { name: "Kanye", description: "The artist with a prophetic role in the cult.", special: "", favProduct: "Ice cream" },
      { name: "Ellen", description: "The talk show host covering up scandals.", special: "", favProduct: "Walnut" }
    ],
    secretLocation: {
      name: "Subterranean Bunker Beneath Paramount Studios",
      description: "A hidden bunker beneath the studio lot where inconvenient actors and insiders have been 'written out' of existence."
    },
    partyLocation: {
      name: "The Playboy Mansion",
      description: "The infamous estate where the elite gather for hedonistic parties and rituals."
    },
    harvestLocation: {
      name: "Foster Homes in Los Angeles",
      description: "The city's network of foster homes where vulnerable children are harvested for the entertainment industry."
    }
  }
]